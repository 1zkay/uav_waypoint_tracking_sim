...
PX4 SITL + Gazebo Harmonic + ROS 2 Jazzy 的本地 NED 航点跟踪仿真工作区。当前视觉链路采用 **YOLO + BoT-SORT 目标跟踪**，并可将跟踪目标保持在主机 `x500_0` 的云台相机画面中心。
...
终端 3，启动主机航点跟踪、相机桥接、YOLO + BoT-SORT 跟踪、日志记录和 RViz 可视化发布节点：
...
`start_waypoint_tracking.sh` 会同时启动控制节点、RViz 可视化发布节点、相机桥接和 YOLO + BoT-SORT 跟踪节点。
...
`scripts/start_waypoint_tracking.sh` 默认启动相机桥接和 YOLO + BoT-SORT 跟踪，权重文件为仓库根目录
