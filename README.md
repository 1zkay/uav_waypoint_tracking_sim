...
- YOLO 检测结果：`/x500_0/yolo/detections`
- YOLO 标注图像：`/x500_0/yolo/image_annotated`

`scripts/start_waypoint_tracking.sh` 默认启动相机桥接和 YOLOv8 检测，权重文件为仓库根目录
`yolov8s.pt`。YOLO Python 依赖当前安装在 `/home/zk/px4-venv`，`scripts/build.sh` 会使用这个
venv 构建 ROS 2 console scripts。只需要航点控制、不需要视觉检测时：

```bash
ENABLE_YOLO_DETECTION=false ./scripts/start_waypoint_tracking.sh
```
...
- `/x500_0/yolo/detections`: 主机云台相机 YOLOv8 检测框，类型为 `vision_msgs/Detection2DArray`。
- `/x500_0/yolo/image_annotated`: YOLOv8 标注后的图像。
